...
from variables import connectionFeedbacks, worksheetFormularioTutorCierre2Ciclo,autocompletarTutor,autocompletarNombre,page_icon
...
def run():
    st.set_page_config(
        page_title="Formulario Fin 2° Ciclo",
        page_icon=page_icon,
        layout="centered",
    )
...
# Load quiz data
with open('content/formulario_tutor_cierre_segundo_ciclo.json', 'r', encoding='utf-8') as f:
...
def save_to_google_sheet(data):
    ...
    existing_data = conn.read(worksheet=worksheetFormularioTutorCierre2Ciclo)
    print(existing_data)
    ...
    conn.update(worksheet=worksheetFormularioTutorCierre2Ciclo, data=updated_data)
