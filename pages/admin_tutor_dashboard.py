...
st.session_state["current_page"] = "admin_tutor_dashboard"
#tab icono y titulo
apply_page_config()
...
df = getInfo()
# Initialize session state for dynamic selection
if "selected_candidato" not in st.session_state:
    st.session_state.selected_candidato = "Todos"
if "selected_tutor" not in st.session_state:
    st.session_state.selected_tutor = "Todos"
if "tutor_updated_manually" not in st.session_state:
    st.session_state.tutor_updated_manually = False 

def update_fields():
    selected_candidato = st.session_state.selected_candidato
    if selected_candidato != "Todos":
        candidato_data = df[df[topFilters[1]] == selected_candidato].iloc[0]
        st.session_state.selected_tutor = candidato_data[topFilters[5]]
        st.session_state.tutor_updated_manually = False 
    else:
        st.session_state.selected_tutor = "Todos"
def manual_tutor_update():
    st.session_state.tutor_updated_manually = True
...
with st.container():
    ...
    with st.container():
        ...
        tutor_options = ["Todos"] + tutor_options
        selected_tutor = st.session_state.selected_tutor if not st.session_state.tutor_updated_manually else None
        tutor = col2.selectbox("**TUTOR**",  tutor_options,index=tutor_options.index(selected_tutor) if selected_tutor in tutor_options else 0, 
               key="selected_tutor", on_change=manual_tutor_update)
    with st.container():
        ...
        candidato = col3.selectbox("**APRENDIZ**", options=["Todos"] + candidato_options, key="selected_candidato",  on_change=update_fields)
    with st.container():
        hotel_options = (
...
print(st.session_state.tutor_updated_manually)
